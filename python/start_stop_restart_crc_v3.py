...
def check_resource(project):
    cmd = f"oc describe quota toto -n {project}"
    result = run_command(cmd)
    return "toto" in result
...
def manage_deployments(mode1, prjndict_filtered, batch_size, used_json_status_before_stop=None, nb_scale_desired=None, filter_mode="exclude",simulate=False):
    failed_deployments = []
            ...
            for i in range(0, len(deployments), batch_size):
                ...
                for deployment in batch:
                    ...
                    if mode1 == "start":
                        ...
                        if status:
                            ...
                            print("Resources not available. Waiting for 90 seconds.")
                            time.sleep(90)
                            status = check_resource(project)
                            if not status:
                                print("Resources still not available. Exiting.")
                                sys.exit(1)
                    elif mode1 == "stop":
                ...
                for deployment in batch:
                    for _ in range(3):
                        ...
                        print("Resources not available after 3 attempts. Waiting for 5 minutes.")
                        time.sleep(300)
                        status = check_resource(project)
                        if not status:
                            print("Resources still not available. Exiting.")
...
output1 = manage_deployments(mode1=mod, prjndict_filtered=prjndict_filtered, batch_size=10, used_json_status_before_stop=used_json_status_before_stop, nb_scale_desired=nb_scale_desired, filter_mode=filter_mode,simulate=simulate)
